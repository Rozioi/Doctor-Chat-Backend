...
const bot = new grammy_1.Bot("8322768927:AAHy9zUG5Li0bFgW6gVYcvNKIQ_2r2bfaIc");
...
bot.callbackQuery("how_it_works", async (ctx) => {
    ...
    await ctx.editMessageText(`🔄 Как работает наша платформа:
...
bot.callbackQuery("payment_guarantee", async (ctx) => {
    await ctx.answerCallbackQuery();
    await ctx.editMessageText(`🛡️ Система гарантий и оплаты:
...
• Защита от недобросовестных пациентов`, {
        parse_mode: "Markdown",
        reply_markup: new grammy_1.InlineKeyboard()
            .webApp("👨‍⚕️ Начать поиск", "https://soundly-primary-protozoa.cloudpub.ru")
            .row()
            .text("🔙 Назад", "back_to_main"),
    });
});
bot.callbackQuery("for_doctors", async (ctx) => {
    await ctx.answerCallbackQuery();
    await ctx.editMessageText(`👨‍⚕️ Для медицинских специалистов:
...
Для регистрации как врач напишите нам: doctors@rozioi.pro`, {
        parse_mode: "Markdown",
        reply_markup: new grammy_1.InlineKeyboard()
            .url("📝 Регистрация врача", "https://soundly-primary-protozoa.cloudpub.ru/doctor-registration")
            .row()
            .text("🔙 Назад", "back_to_main"),
    });
});
bot.callbackQuery("contacts", async (ctx) => {
    await ctx.answerCallbackQuery();
    await ctx.editMessageText(`<b>📞 Контактная информация</b>
...
Мы всегда на связи!`, {
        parse_mode: "HTML",
        reply_markup: new grammy_1.InlineKeyboard()
            .webApp("👨‍⚕️ Перейти к платформе", "https://soundly-primary-protozoa.cloudpub.ru")
            .row()
            .text("🔙 Назад", "back_to_main"),
    });
});
bot.callbackQuery("back_to_main", async (ctx) => {
    await ctx.answerCallbackQuery();
    await ctx.editMessageText(`👋 С возвращением на медицинскую платформу Doctor Chat!
...
bot.on("message", async (ctx) => {
    ...
    if (!message)
        return;
    if (message.includes("врач") ||
        message.includes("специалист") ||
        message.includes("доктор")) {
        await ctx.reply("🔍 Найдем подходящего врача для вас! Используйте нашу платформу для безопасного подключения.", {
            reply_markup: new grammy_1.InlineKeyboard().webApp("👨‍⚕️ Найти врача", "https://soundly-primary-protozoa.cloudpub.ru"),
        });
    }
    else if (message.includes("оплат") ||
        message.includes("гарант") ||
        message.includes("безопасн")) {
        await ctx.reply("🛡️ Мы гарантируем безопасность платежей! Средства хранятся у нас до подтверждения качества консультации.", {
            reply_markup: new grammy_1.InlineKeyboard().text("💳 Подробнее о гарантиях", "payment_guarantee"),
        });
    }
    else if (message.includes("я врач") ||
        message.includes("медик") ||
        message.includes("специалист")) {
        await ctx.reply("👨‍⚕️ Отлично! Для подключения к платформе как врач напишите на doctors@rozioi.pro или посетите наш сайт.", {
            reply_markup: new grammy_1.InlineKeyboard().url("📝 Регистрация врача", "https://soundly-primary-protozoa.cloudpub.ru/doctor-registration"),
        });
    }
    else if (message.includes("помощь") || message.includes("help")) {
        await ctx.reply("Используйте кнопки меню для навигации или напишите /start");
    }
    else {
        await ctx.reply("🔍 Чем могу помочь? Используйте нашу платформу для безопасного подключения к врачам 👇", {
            reply_markup: new grammy_1.InlineKeyboard().webApp("👨‍⚕️ Перейти к платформе", "https://soundly-primary-protozoa.cloudpub.ru"),
        });
    }
});
bot.command("help", async (ctx) => {
    await ctx.reply(`❓ Помощь по платформе:
